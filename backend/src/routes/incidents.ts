...
function parseStatus(value: string | undefined): IncidentStatus | undefined {
  ...
  const upper = value.toUpperCase();
  if (upper === "OPEN" || upper === "CONTAINED" || upper === "RESOLVED" || upper === "ARCHIVED") {
...
router.get("/stats", async (_req, res, next) => {
  try {
    ...
    const byStatus: Record<string, number> = { OPEN: 0, CONTAINED: 0, RESOLVED: 0, ARCHIVED: 0 };
